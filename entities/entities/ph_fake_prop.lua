...
if SERVER then	
		...
		if GAMEMODE:InRound() and attacker:IsPlayer() and attacker:Team() == TEAM_HUNTERS and dmg:GetDamage() > 0 then
			...
			if self.health <= 0 then
                ...
                if IsValid(owner) or owner ~= NULL then
                    ...
                    owner:AddFrags( 1 )
                    -- Remove from decoys table
                    if owner.propdecoys then
                        for i, d in ipairs(owner.propdecoys) do
                            if d == self then
                                table.remove(owner.propdecoys, i)
                                break
                            end
                        end
    ...
    -- Remove all decoys on Round End
    hook.Add("PH_RoundEndResult", "PHX.DestroyDecoys", function(r,rt)
        for _,v in pairs(player.GetAll()) do
            -- Clean up decoys table
            if v.propdecoys then
                for _, decoy in ipairs(v.propdecoys) do
