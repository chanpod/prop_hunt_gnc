...
-- GNC: Panic Mode - faster auto-taunt near round end
CVAR["ph_autotaunt_panic_enabled"]		=	{ CTYPE_BOOL, 	"1", CVAR_SERVER_ONLY_NO_NOTIFY, "Enable panic mode (faster auto-taunt when round is almost over)" }
CVAR["ph_autotaunt_panic_threshold"]	=	{ CTYPE_NUMBER, "60", CVAR_SERVER_ONLY_NO_NOTIFY, "Seconds remaining to trigger panic mode", { min = 10, max = 180 } }
CVAR["ph_autotaunt_panic_delay"]		=	{ CTYPE_NUMBER, "10", CVAR_SERVER_ONLY_NO_NOTIFY, "Auto-taunt delay during panic mode", { min = 3, max = 30 } }
-- GNC: Ready-Up System - require players to ready before round starts
CVAR["ph_readyup_enabled"]				=	{ CTYPE_BOOL, 	"1", CVAR_SERVER_ONLY_NO_NOTIFY, "Enable ready-up system (players must ready before first round)" }
CVAR["ph_readyup_threshold"]			=	{ CTYPE_NUMBER, "80", CVAR_SERVER_ONLY_NO_NOTIFY, "Percentage of players that must ready to start countdown", { min = 50, max = 100 } }
CVAR["ph_readyup_countdown"]			=	{ CTYPE_NUMBER, "20", CVAR_SERVER_ONLY_NO_NOTIFY, "Countdown duration after threshold reached (seconds)", { min = 5, max = 60 } }
...
-- GNC: Stationary Taunt - increased taunt frequency and duration when not moving
CVAR["ph_exp_stationary_taunt_enabled"]		=	{ CTYPE_BOOL, 	"0", 	CVAR_SERVER_ONLY_NO_NOTIFY, "[Experimental] Enable increased taunt frequency when props are stationary." }
CVAR["ph_exp_stationary_threshold"]			=	{ CTYPE_NUMBER, "30", 	CVAR_SERVER_ONLY_NO_NOTIFY, "Seconds of not moving before considered stationary.", { min = 5, max = 120 } }
CVAR["ph_exp_stationary_min_duration"]		=	{ CTYPE_FLOAT, 	"5.0", 	CVAR_SERVER_ONLY_NO_NOTIFY, "Minimum taunt duration (seconds) when stationary.", { min = 1, max = 30 } }
CVAR["ph_exp_stationary_delay_mult"]		=	{ CTYPE_FLOAT, 	"0.5", 	CVAR_SERVER_ONLY_NO_NOTIFY, "Taunt delay multiplier when stationary (0.5 = twice as often).", { min = 0.1, max = 1.0 } }

-- GNC: Proximity Panic - auto-taunt when running near hunters
CVAR["ph_exp_proximity_panic_enabled"]		=	{ CTYPE_BOOL, 	"0", 	CVAR_SERVER_ONLY_NO_NOTIFY, "[Experimental] Auto-play panic taunt when prop runs near a hunter." }
CVAR["ph_exp_proximity_panic_distance"]		=	{ CTYPE_NUMBER, "200", 	CVAR_SERVER_ONLY_NO_NOTIFY, "Distance (units) to trigger proximity panic. ~160 = 10ft.", { min = 50, max = 500 } }
