...
-- GNC: Panic Mode variables
local GNC_PanicMode = false
local GNC_PanicAnnounced = false

-- GNC: Stationary Mode variable
local GNC_StationaryMode = false

-- GNC: Get remaining round time
local function GNC_GetRoundTimeRemaining()
	local roundEndTime = GetGlobalFloat("RoundEndTime", 0)
	if roundEndTime == 0 then return 999 end
	return roundEndTime - CurTime()
end

-- GNC: Check if panic mode should be active
local function GNC_IsPanicModeActive()
	if not PHX:GetCVar("ph_autotaunt_panic_enabled") then return false end
	local threshold = PHX:GetCVar("ph_autotaunt_panic_threshold") or 60
	local timeLeft = GNC_GetRoundTimeRemaining()
	return timeLeft > 0 and timeLeft <= threshold
end

-- GNC: Check if player is stationary (networked from server)
local function GNC_IsStationaryModeActive()
	if not PHX:GetCVar("ph_exp_stationary_taunt_enabled") then return false end
	local ply = LocalPlayer()
	if not IsValid(ply) then return false end
	return ply:GetNWBool("PHX.IsStationary", false)
end

-- GNC: Get effective delay based on panic mode and stationary mode
local function GNC_GetEffectiveDelay()
	local baseDelay = PHX:GetCVar("ph_autotaunt_delay") or 45

	-- Panic mode takes priority
	if GNC_IsPanicModeActive() then
		GNC_PanicMode = true
		GNC_StationaryMode = false
		return PHX:GetCVar("ph_autotaunt_panic_delay") or 10
	end
	GNC_PanicMode = false

	-- Check stationary mode
	if GNC_IsStationaryModeActive() then
		GNC_StationaryMode = true
...
local function TimeLeft()
	...
	local effectiveDelay = GNC_GetEffectiveDelay()
	local nextTauntTime = lastTauntTime + effectiveDelay
...
local function AutoTauntPaint()
	...
	local effectiveDelay = GNC_GetEffectiveDelay()
	local percentage = timeLeft / effectiveDelay

	-- GNC: Show panic/stationary mode text
	local txt = PHX:FTranslate("HUD_AUTOTAUNT_ALT", timeLeft)
	local barColor = Color(200, 0, 0, 200)
	if GNC_PanicMode then
		txt = "PANIC! " .. timeLeft .. "s"
		barColor = Color(255, 100, 0, 200)
	elseif GNC_StationaryMode then
		txt = "STATIONARY! " .. timeLeft .. "s"
		barColor = Color(150, 50, 200, 200)
	end

	draw.RoundedBox(5, xElastic, y, w, h, Color(0, 0, 0, 200))
	draw.RoundedBox(5, xElastic + 5, y + 5, (w - 10) * percentage, h - 10, barColor)
	draw.DrawText(txt, "HunterBlindLockFont", xElastic + 70, ScrH() - 57, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER)

	-- GNC: Flashing panic indicator
	if GNC_PanicMode then
		local alpha = math.abs(math.sin(CurTime() * 4)) * 255
		draw.SimpleText("!! PANIC MODE !!", "DermaLarge", ScrW() / 2, 80, Color(255, 100, 0, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	elseif GNC_StationaryMode then
		local alpha = math.abs(math.sin(CurTime() * 2)) * 200 + 55
		draw.SimpleText("!! STATIONARY - MOVE! !!", "DermaLarge", ScrW() / 2, 80, Color(150, 50, 200, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
end
...
local function AutoTauntPaint_phx()
	...
	if IsValid(LocalPlayer()) && LocalPlayer():Alive() && LocalPlayer():Team()==TEAM_PROPS && started then
		...
		if PHX:GetCVar( "ph_autotaunt_enabled" ) then
			-- GNC: Show panic/stationary mode in text
			if GNC_PanicMode then
				taunttext = "PANIC! " .. timeLeft .. "s"
			elseif GNC_StationaryMode then
				taunttext = "STATIONARY! " .. timeLeft .. "s"
			else
			...
			-- GNC: Color for panic/stationary mode
			if GNC_PanicMode then
				surface.SetDrawColor( Color(255,140,0) )
			elseif GNC_StationaryMode then
				surface.SetDrawColor( Color(150,50,200) )
			else
		...
		-- GNC: Colored text for panic/stationary mode
		local textColor = color_white
		if GNC_PanicMode then
			textColor = Color(255, 200, 0)
		elseif GNC_StationaryMode then
			textColor = Color(200, 150, 255)
		end
		draw.DrawText( taunttext, "PHX.TopBarFont", ScrW() - 164, posw.y + 83, textColor, TEXT_ALIGN_RIGHT )

		-- GNC: Flashing panic/stationary indicator
		if GNC_PanicMode then
			local alpha = math.abs(math.sin(CurTime() * 4)) * 255
			draw.SimpleText("!! PANIC MODE !!", "DermaLarge", ScrW() / 2, 80, Color(255, 100, 0, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		elseif GNC_StationaryMode then
