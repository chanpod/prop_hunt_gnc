...
-- Multi-decoy system: get count of active decoys
function Player:GetDecoyCount()
	if not self.propdecoys then return 0 end
	-- Clean up invalid entries and count
	local count = 0
	for i = #self.propdecoys, 1, -1 do
		if not IsValid(self.propdecoys[i]) then
			table.remove(self.propdecoys, i)
		else
			count = count + 1
		end
	end
...
if SERVER then
	...
	function Player:PlaceDecoyProp()
		...
		if self:CanPlaceDecoy() and self:Alive() and self:Team() == TEAM_PROPS then
			local trace = {}
			...
			if (tr.Hit and tr.HitNormal.z > 0.5) then
				...
				-- Initialize decoys table if needed
				if not self.propdecoys then self.propdecoys = {} end

				local newdecoy = ents.Create("ph_fake_prop")
                if self.ph_prop:GetModel() == "models/player/kleiner.mdl" or
                    ...
                    newdecoy:SetPos( pos - Vector(0,0, self.ph_prop:OBBMins().z ) )
                end
				newdecoy:SetAngles( self.ph_prop:GetAngles() )
                newdecoy:SetfEntityColor( Vector( playercolor ) )
				newdecoy:Spawn()

				newdecoy:ChangeModel( self.ph_prop )
                newdecoy:SetOwner( self )

				-- Add to decoys table
				table.insert(self.propdecoys, newdecoy)
	...
	function Player:RemoveDecoyByEntity(decoy)
		if not self.propdecoys then return end
		for i, d in ipairs(self.propdecoys) do
			if d == decoy then
				table.remove(self.propdecoys, i)
				break
			end
	...
	function Player:DeploySmokeScreen()
		...
		effectData:SetMagnitude(duration)
